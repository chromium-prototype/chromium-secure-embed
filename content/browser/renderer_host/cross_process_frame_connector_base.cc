...
void CrossProcessFrameConnectorBase::SetView(
  ...
  if (view_) {
    ...
    ReportFrameSinkId(view_->GetFrameSinkId(), allow_paint_holding);
  }
}

void CrossProcessFrameConnectorBase::RenderProcessGone() {
  OPTIONAL_TRACE_EVENT1("content",
                        "CrossProcessFrameConnectorBase::RenderProcessGone",
  ...
  for (auto* rfh = current_child_rfh->GetParentOrOuterDocumentOrEmbedder(); rfh;
    ...
    if (rfh->GetProcess()->GetDeprecatedID() == process_id) {
      ...
      // the current instance of the CrossProcessFrameConnectorBase.
...
void CrossProcessFrameConnectorBase::SynchronizeVisualProperties(
...
CrossProcessFrameConnectorBase::GetParentViewInput() {
  return GetParentRenderWidgetHostView();
}

input::RenderWidgetHostViewInput*
CrossProcessFrameConnectorBase::GetRootViewInput() {
  return GetRootRenderWidgetHostView();
}

double CrossProcessFrameConnectorBase::GetCssZoomFactor() const {
  return last_received_css_zoom_factor_;
}

const gfx::Size& CrossProcessFrameConnectorBase::GetLocalFrameSizeInPixels()
    const {
  return local_frame_size_in_pixels_;
}

const gfx::Size& CrossProcessFrameConnectorBase::GetLocalFrameSizeInDip()
    const {
  return local_frame_size_in_dip_;
}

const gfx::Rect& CrossProcessFrameConnectorBase::GetRectInParentViewInDip()
    const {
  return rect_in_parent_view_in_dip_;
}

uint32_t CrossProcessFrameConnectorBase::GetCaptureSequenceNumber() const {
  return capture_sequence_number_;
}

void CrossProcessFrameConnectorBase::UpdateCursor(const ui::Cursor& cursor) {
  RenderWidgetHostViewBase* root_view = GetRootRenderWidgetHostView();
...
CrossProcessFrameConnectorBase::HasFocus() {
  RenderWidgetHostViewBase* root_view = GetRootRenderWidgetHostView();
  if (!root_view) {
...
void CrossProcessFrameConnectorBase::FocusRootView() {
  RenderWidgetHostViewBase* root_view = GetRootRenderWidgetHostView();
  if (root_view) {
    root_view->Focus();
  }
...
blink::mojom::PointerLockResult CrossProcessFrameConnectorBase::LockPointer(
...
CrossProcessFrameConnectorBase::ChangePointerLock(
    bool request_unadjusted_movement) {
  RenderWidgetHostViewBase* root_view = GetRootRenderWidgetHostView();
  if (root_view) {
...
void CrossProcessFrameConnectorBase::UnlockPointer() {
  RenderWidgetHostViewBase* root_view = GetRootRenderWidgetHostView();
  if (root_view) {
...
CrossProcessFrameConnectorBase::GetIntersectionState() const {
  return intersection_state_;
}

const viz::LocalSurfaceId& CrossProcessFrameConnectorBase::GetLocalSurfaceId()
    const {
  return local_surface_id_;
}

const display::ScreenInfos& CrossProcessFrameConnectorBase::GetScreenInfos()
    const {
  return screen_infos_;
}

void CrossProcessFrameConnectorBase::OnSynchronizeVisualProperties(
    const blink::FrameVisualProperties& visual_properties) {
  TRACE_EVENT_WITH_FLOW2(
      TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"),
      "CrossProcessFrameConnectorBase::OnSynchronizeVisualProperties Receive "
      "Message",
...
void CrossProcessFrameConnectorBase::UpdateViewportIntersection(
...
void CrossProcessFrameConnectorBase::UpdateViewportIntersectionInternal(
    const blink::mojom::ViewportIntersectionState& intersection_state,
...
void CrossProcessFrameConnectorBase::OnVisibilityChanged(
  ...
  if (NotifyInnerWebContentsVisibilityChanged()) {
    return;
  }
...
void CrossProcessFrameConnectorBase::SetIsInert(bool inert) {
  is_inert_ = inert;
  if (view_) {
    view_->SetIsInert();
  }
...
void CrossProcessFrameConnectorBase::OnSetInheritedEffectiveTouchAction(
...
bool CrossProcessFrameConnectorBase::IsInert() const {
  return is_inert_;
}

cc::TouchAction CrossProcessFrameConnectorBase::InheritedEffectiveTouchAction()
    const {
  return inherited_effective_touch_action_;
}

bool CrossProcessFrameConnectorBase::IsHidden() const {
  return visibility_ == blink::mojom::FrameVisibility::kNotRendered;
}

bool CrossProcessFrameConnectorBase::HasSize() const {
  return has_size_;
}

void CrossProcessFrameConnectorBase::SetVisibilityForChildViews(
    bool visible) const {
  current_child_frame_host()->SetVisibilityForChildViews(visible);
}

void CrossProcessFrameConnectorBase::SetLocalFrameSize(
...
void CrossProcessFrameConnectorBase::SetRectInParentView(
  ...
  if (view_ && current_child_frame_host()) {
    ...
    // TODO(secure-embed): This change may need attention in reivew.
    FrameTreeNode* proxy_node = current_child_frame_host()->frame_tree_node();
...
void CrossProcessFrameConnectorBase::ResetRectInParentView() {
...
void CrossProcessFrameConnectorBase::UpdateRenderThrottlingStatus(
    bool is_throttled,
...
bool CrossProcessFrameConnectorBase::IsThrottled() const {
  return is_throttled_;
}

bool CrossProcessFrameConnectorBase::IsSubtreeThrottled() const {
  return subtree_throttled_;
}

bool CrossProcessFrameConnectorBase::IsDisplayLocked() const {
  return display_locked_;
}

bool CrossProcessFrameConnectorBase::MaybeLogCrash(CrashVisibility visibility) {
  if (!has_crashed_) {
    return false;
  }
...
void CrossProcessFrameConnectorBase::MaybeLogShownCrash(
...
void CrossProcessFrameConnectorBase::DelegateWasShown() {
  if (IsVisible()) {
    ...
    MaybeLogShownCrash(
        CrossProcessFrameConnectorBase::ShownAfterCrashingReason::kTabWasShown);
  }
...
bool CrossProcessFrameConnectorBase::IsVisible() {
...
Visibility CrossProcessFrameConnectorBase::EmbedderVisibility() {
