...
//
// TODO(secure-embed): Reorganize comments.
class CONTENT_EXPORT CrossProcessFrameConnectorBase
    : public input::ChildFrameInputHelper::Delegate {
 public:
  CrossProcessFrameConnectorBase(const CrossProcessFrameConnectorBase&) =
      delete;
  CrossProcessFrameConnectorBase& operator=(
      const CrossProcessFrameConnectorBase&) = delete;

  ~CrossProcessFrameConnectorBase() override;
  ...
  // TODO(secure-embed): Comments need update.
  ...
  double GetCssZoomFactor() const;

  // Return the size of the CompositorFrame to use in the child renderer.
  const gfx::Size& GetLocalFrameSizeInPixels() const;
  ...
  const gfx::Size& GetLocalFrameSizeInDip() const;

  // Return the rect in DIP that the RenderWidgetHostViewChildFrame's content
  // will render into.
  const gfx::Rect& GetRectInParentViewInDip() const;
  ...
  const blink::mojom::ViewportIntersectionState& GetIntersectionState() const;
  ...
  // child frame.
  const display::ScreenInfos& GetScreenInfos() const;
 ...
 protected:
  ...
  // TODO(secure-embed): Data members shouldn't be protected, some may need
  // getters.
  ...
  // Implemented in subclass, with Report* method notifying the parent:
  ...
  // TODO(secure-embed): Update comment
  virtual RenderFrameHostImpl* current_child_frame_host() const = 0;
  virtual void ReportFrameSinkId(const viz::FrameSinkId& frame_sink_id,
                                 bool allow_paint_holding) = 0;
  virtual void ReportChildProcessGone() = 0;
  virtual void ReportBadVisualProperties() = 0;
  virtual bool NotifyInnerWebContentsVisibilityChanged() = 0;
  virtual void RequestTabReload() = 0;
